    ...
    <div className="flex flex-col gap-5">
      {work.map((item) => (
        <div key={item.id} className="px-4 py-6 bg-[#FFFFFF0A] rounded-xl">
          <div className="text-sm font-normal text-gray-white md:text-base">
            {item.date}
          </div>
          <div className="text-2xl font-medium text-gray-white my-2 md:text-3xl">
            {item.organization}
          </div>
          {item.responsibilities.map((responsibility) => (
            <div key={responsibility.id}>
              <li className="text-sm font-normal text-gray-white mb-2 md:text-lg">
                {responsibility.desc}
              </li>
            </div>
          ))}
